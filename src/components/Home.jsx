...
const subtext = ['This is the subtext', 'Next subtext']

const useTypedSubtext = (subtext: string[]) => {
    const [typedSubtext, setTypedSubtext] = useState('')
    useEffect(() => {
     const nextTypedSubtext = subtext.slice(0, typedSubtext.length + 1)
  
     if (nextTypedSubtext === typedSubtext) return
  
     const timeout = setTimeout(() => {
          setTypedSubtext(subtext.slice(0, typedSubtext.length + 1))
      }, 150)
...
const Home = () => {
  const subtext = useTypedSubtext(subtext)
  const [typedSubtext, setTypedSubtext] = useState('')
  useEffect(() => {
   const nextTypedSubtext = subtext.slice(0, typedSubtext.length + 1)
  ...
  return (
    <div name='home' className='w-full h-screen bg-gradient-to-r from-[#2A5470] to-[#4C4177]'>
        ...
        <div className='max-w-[1000px] mx-auto px-8 flex flex-col justify-center h-full'>
            ...
            <h2 className='text-4xl py-2 sm:text-7xl font-bold text-[#8892b0] blinking-cursor'>{typedSubtext}</h2>
